...
describe('exercisesService', () => {
  ...
  describe('createCustomExercise', () => {
    ...
    it('returns the created exercise when successful', async () => {
      jest.spyOn(axiosInstance, 'post').mockResolvedValueOnce({
        data: mockExercises[0]
      });

      const result = await exercisesService.createCustomExercise(payload);

      expect(axiosInstance.post).toHaveBeenCalledWith('/exercises', payload);
    ...
    it('throws API error responses when available', async () => {
      ...
      await expect(
        exercisesService.createCustomExercise(payload)
